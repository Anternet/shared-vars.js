...
class Peer {
  ...
  constructor(rinfo) {
    if (typeof rinfo !== 'string') {
      this.address = this.constructor.normalizeAddress(rinfo.address);
      this.port = rinfo.port;
      this.family = rinfo.family;
    } else {
      const i = rinfo.lastIndexOf(':');

      this.address = this.constructor.normalizeAddress(rinfo.substr(0, i));
  ...
  static equals(a, b) {
    return (ip.isEqual(a.address, b.address) && a.port === b.port);
  }

  static rinfoKey(rinfo) {
    const address = this.normalizeAddress(rinfo.address);
    return `${address}:${rinfo.port}`;
  }
