...
class SharedVar extends EventEmitter {
  ...
  pushUpdate(signature) {
    if (this.signature) {
      if (!this.signature.similarTo(signature)) throw new Error('The given signature is not the same as current signature');

      if (this.signature.betterThen(signature)) return false;
    }
    ...
    this._shared.download(signature, preferredPeers, (err, value) => {
      ...
      this.emit('update', signature.timestamp);
